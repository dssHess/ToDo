...
class C_ToDo(Hauptdatenbank):
    ...
    db_t_erinnerung = Column(DateTime)                 # Wann soll ich daran erinnerung werden (datum und Zeit)
    db_t_bemerkung = Column(String)                  # Bemerkung im Klartext (str)
    db_t_faellig = Column(DateTime)                    # Wann soll die Aufgabe erledigt werden (datum und Zeit)
    db_t_erledigt = Column(DateTime)                   # Ist die Aufgabe erledigt (datum und Zeit)
...
def F_neue_aufgabe():
    ...
    n_erinnerung  = input("Wann sol ich dich daran erinnern? (Datum und Zeit)\t: ")
    n_bemerkung    = input("Bitte geben eine Bemrkungen ein\t: ")
    n_faellig      = input("Wann ist die Aufgabe fällig? (Datum und Zeit)\t: ")

    n_erinnerung  = datetime.strptime(n_erinnerung, "%d.%m.%Y %H:%M")
    n_faellig     = datetime.strptime(n_faellig, "%d.%m.%Y %H:%M")
    

    n_neue_aufgabe = C_ToDo(
        db_t_aufgabe=n_aufgabe, 
        db_t_erinnerung=n_erinnerung, 
        db_t_bemerkung=n_bemerkung, 
        db_t_faellig=n_faellig
        )
...
def F_loesche_aufgabe():
    ...
    console.print(f"Lösche todo {f_todo.db_t_aufgabe} {f_todo.db_t_erinnerung}.", style="red")
...
def F_Zeige_alle_aufgaben():
    ...
    table.add_column("db_t_erinnerung")
    table.add_column("db_t_bemerkung")
    table.add_column("db_t_faellig")
    
    for f_todo in f_aufgaben:
        neu1 =  str(f_todo.db_t_erinnerung)
        neu2 =  str(f_todo.db_t_faellig)
        table.add_row(str(f_todo.db_t_todo_id),f_todo.db_t_aufgabe,neu1,f_todo.db_t_bemerkung,neu2)
...
def F_veraendere_eine_aufgabe():
    ...
    f_neuer_wert_erinnerung = input(f"erinnerung \t[{f_todo_felder.db_t_erinnerung}]: ")
    if f_neuer_wert_erinnerung:
        f_todo_felder["f_todo_felder.db_t_erinnerung"] = f_neuer_wert_erinnerung
